...
void LinkMappingVersion::addAutogeneratedLink(const std::string& srcProvinceID, const std::string& tgtProvinceID)
{
	...
	if (link && tgtProvinceID == "7210") // TODO: REMOVE THIS
	{
		Log(LogLevel::Error) << "Using existing SRC link from source province " << srcProvinceID << " to target province " << tgtProvinceID;
	}
	...
	if (!link)
	{
		...
		if (link && tgtProvinceID == "7210") // TODO: REMOVE THIS
		{
			Log(LogLevel::Error) << "Using existing TGT link from source province " << srcProvinceID << " to target province " << tgtProvinceID; //
		}
	}
	...
	if (!link)
	{
		if (tgtProvinceID == "7210") // TODO: REMOVE THIS
		{
			Log(LogLevel::Error) << "Creating new link from source province " << srcProvinceID << " to target province " << tgtProvinceID;
		}
		link = std::make_shared<LinkMapping>(sourceDefs, targetDefs, sourceToken, targetToken, linkCounter, true);
		++linkCounter;
		const auto& positionItr = links->begin() + lastActiveLinkIndex;
		links->insert(positionItr, link);
	}
...
void LinkMappingVersion::deleteLinkByID(const int theID)
{
	...
	for (const auto& link: *links)
	{
		if (link->getID() == theID)
		{
			...
			for (const auto& province: link->getSources())
			{
				// If province already belongs to unmappedSources, don't add it again.
				bool add = true;
				for (const auto& unmappedProvince: *unmappedSources)
				{
					if (unmappedProvince->ID == province->ID)
					{
						add = false;
						break;
					}
			...
			for (const auto& province: link->getTargets())
			{
				...
				bool add = true;
				for (const auto& unmappedProvince: *unmappedTargets)
				{
					if (unmappedProvince->ID == province->ID)
					{
						add = false;
