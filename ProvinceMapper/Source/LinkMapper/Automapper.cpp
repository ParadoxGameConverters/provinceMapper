...
void Automapper::generateLinks()
{
	...
	for (const auto& [tgtProvID, srcProvMatches]: targetProvinceShares)
	{
		...
		Log(LogLevel::Info) << "<> Automapping target province " << tgtProvID << " ..."; // TODO: REMOVE THIS
		Log(LogLevel::Info) << "-----> Source provinces: " << srcProvMatches.size();		// TODO: REMOVE THIS
		...
		{
			Log(LogLevel::Info) << "--------> Source province " << srcProvinceID << " not available";
			continue;
		}

		activeVersion->addAutogeneratedLink(srcProvinceID, tgtProvID);
	...
	for (const auto& [srcProvID, tgtProvMatches]: sourceProvinceShares)
	{
		...
		activeVersion->addAutogeneratedLink(srcProvID, tgtProvinceID);

		srcProvincesToRemove.insert(srcProvID);
	}
	cleanUpSourceProvinceShares();
	Log(LogLevel::Debug) << "Automapping step 2 complete.";
	...
	commentID = activeVersion->addRawComment();				// todo: remove this
	activeVersion->activateLinkByID(commentID);				// todo: remove this
	activeVersion->getActiveLink()->setComment("STEP 3"); // todo: remove this
	for (const auto& [tgtProvID, srcProvMatches]: targetProvinceShares)
	{
		if (tgtImpassablesCache.contains(tgtProvID))
		{
	...
	cleanUpTargetProvinceShares();
	Log(LogLevel::Debug) << "Automapping step 3 complete.";
	...
	commentID = activeVersion->addRawComment();				// todo: remove this
	activeVersion->activateLinkByID(commentID);				// todo: remove this
	activeVersion->getActiveLink()->setComment("STEP 4"); // todo: remove this
	for (const auto& [srcProvID, tgtProvMatches]: sourceProvinceShares)
	{
		if (srcImpassablesCache.contains(srcProvID))
		{
	...
	cleanUpSourceProvinceShares();
	Log(LogLevel::Debug) << "Automapping step 4 complete.";

	// 5. For all yet unmapped target provinces:
	//    Try to use the most matching available source province to map them.
	commentID = activeVersion->addRawComment();				// todo: remove this
	activeVersion->activateLinkByID(commentID);				// todo: remove this
	activeVersion->getActiveLink()->setComment("STEP 5"); // todo: remove this
	for (const auto& [tgtProvID, srcProvMatches]: targetProvinceShares)
	{
		auto highestSrcMatches = getHighestMatches(srcProvMatches);
		for (const auto& [share, srcProvID]: highestSrcMatches)
	...
	cleanUpTargetProvinceShares();
	Log(LogLevel::Debug) << "Automapping step 5 complete.";

	// 6. For all yet unmapped source provinces:
	//    Try to use the most matching available target province to map them.
	commentID = activeVersion->addRawComment();				// todo: remove this
	activeVersion->activateLinkByID(commentID);				// todo: remove this
	activeVersion->getActiveLink()->setComment("STEP 6"); // todo: remove this
	for (const auto& [srcProvID, tgtProvMatches]: sourceProvinceShares)
	{
		auto highestTgtMatches = getHighestMatches(tgtProvMatches);
		for (const auto& [share, tgtProvID]: highestTgtMatches)
	...
	cleanUpSourceProvinceShares();
	Log(LogLevel::Debug) << "Automapping step 6 complete.";
