...
LinkMapping::LinkMapping(std::istream& theStream,
	 ...
	 const int theID):
	 LinkBase(theID), sourceDefs(std::move(theSourceDefs)), targetDefs(std::move(theTargetDefs)), sourceToken(std::move(theSourceToken)),
...
LinkMapping::LinkMapping(std::shared_ptr<DefinitionsInterface> theSourceDefs,
	 ...
	 std::string theSourceToken,
	 std::string theTargetToken,
	 const int theID): LinkMapping(std::move(theSourceDefs), std::move(theTargetDefs), std::move(theSourceToken), std::move(theTargetToken), theID, false)
{
}
...
LinkMapping::LinkMapping(std::shared_ptr<DefinitionsInterface> theSourceDefs,
	 ...
	 std::string theTargetToken,
	 const int theID,
	 const bool autogenerated):
	 LinkBase(theID), sourceDefs(std::move(theSourceDefs)), targetDefs(std::move(theTargetDefs)), sourceToken(std::move(theSourceToken)),
	 targetToken(std::move(theTargetToken)), autogenerated(autogenerated)
{
}
...
void LinkMapping::registerKeys(commonItems::parser& parser)
{
	...
	parser.registerKeyword("autogenerated", [this](std::istream& theStream) {
		autogenerated = commonItems::getString(theStream) == "yes";
	});
...
std::ostream& operator<<(std::ostream& output, const LinkMapping& linkMapping)
{
	...
	// If the link is autogenerated, output the flag.
	if (linkMapping.autogenerated)
	{
		output << "autogenerated = yes ";
	}
...
void LinkMapping::addSource(const std::string& sourceID)
{
	// If already present in sources, do nothing.
	bool found = false;
	...
	if (!found)
	{
		if (const auto province = sourceDefs->getProvinceForID(sourceID))
		{
			sources.emplace_back(province);
		}
		else
		{
			Log(LogLevel::Warning) << "Province ID " << sourceID << " not found in source definitions.";
		}
	}
}

void LinkMapping::addTarget(const std::string& targetID)
{
	// If already present in targets, do nothing.
	bool found = false;
	...
	if (!found)
	{
		if (const auto province = targetDefs->getProvinceForID(targetID))
		{
			targets.emplace_back(province);
		}
		else
		{
			Log(LogLevel::Warning) << "Province ID " << targetID << " not found in target definitions.";
		}
	}
}
